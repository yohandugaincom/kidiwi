...
if ($secure == true) {
  ...
  while ($donnees = $req->fetch()) {
    $email_adress = $donnees['email_expediteur'];
  }
  if ($email_adress == $_SESSION['email']) {
    $requete = "UPDATE ".BDD."messages SET visible_exp='non' WHERE id='".$_GET['id']."'";
    $pdo->prepare($requete)->execute();
    header('Location: /public/compte/prive/messagerie-envoye.php');
  }
}
?>
